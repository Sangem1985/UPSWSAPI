...
namespace UPSWSWebAPIs.DataLayer.Implementations
{
    public class JWTAuthTokenRepository: IJWTAuthTokenRepository
    {
        ...
        public JWTAuthTokenRepository(IConfiguration configuration, HttpClient httpClient, IMemoryCache memoryCache)
        {
            ...
            this._connectionString = configuration.GetConnectionString("DefaultConnString")
        ...
        public async Task<string> GetSecureDataAsOutPut()
        {
            ...
            try
            {
                ...
                var result = await connection.QueryAsync("select * from employees;");

                var retValdata = JsonSerializer.Serialize(result?.FirstOrDefault());
                return retValdata;
            }
            catch (Exception ex)
            {
